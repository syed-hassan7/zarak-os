      ...
      <div className="p-7 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 w-full max-w-7xl mx-auto">
        {apps.map((app, index) => (
          <motion.button
            ...
            className="group flex aspect-square w-full flex-col items-center justify-center gap-2.5 rounded-2xl border border-transparent outline-none transition-transform duration-150 ease-out hover:-translate-y-0.5 focus-visible:border-white/20 focus-visible:ring-2 focus-visible:ring-white/40 motion-reduce:transition-none motion-reduce:hover:translate-y-0"
          >
            <div className="flex h-[64px] w-[64px] min-[1920px]:h-20 min-[1920px]:w-20 transform-gpu items-center justify-center rounded-[18px] min-[1920px]:rounded-[22px] border border-white/12 bg-white/[0.09] text-os-text-pri shadow-lg shadow-black/20 transition-[transform,box-shadow,border-color] duration-150 ease-out will-change-transform group-hover:scale-105 group-hover:border-white/24 group-hover:shadow-[0_0_16px_rgba(45,212,191,0.14),0_8px_24px_rgba(0,0,0,0.3)] motion-reduce:transition-none motion-reduce:group-hover:scale-100">
              <app.icon size={32} strokeWidth={1.5} className="min-[1920px]:w-10 min-[1920px]:h-10" />
            </div>
            <span className="wrap-break-word line-clamp-2 rounded-md bg-black/40 px-2 py-0.5 text-center font-mono text-[11px] min-[1920px]:text-[13px] tracking-tight text-os-text-sec transition-colors duration-150 group-hover:text-os-text-pri motion-reduce:transition-none">
              {app.label}
            </span>
          </motion.button>
        ))}
